...
async function searchStudents(q: string, filter: string): Promise<Result[]> {
  ...
  return (data ?? []).map((s: any) => ({
    id: s.person_id,
    cells: [s.code ?? '—', s.first_name ?? '—', s.surname ?? '—'],
  }))
}
  ...
  return (versions ?? [])
    ...
    .map((o: any) => ({
      ...
      cells: [
        o.code ?? '—',
        o.title ?? '—',
        o.start_timestamp ? new Date(o.start_timestamp).toLocaleDateString('en-AU') : '—',
      ],
    }))
}
...
export default function SearchWidget({ type }: { type: WidgetType }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const data = type === 'students'
        ? await searchStudents(search, settings.filter)
        : await searchOfferings(search, settings.filter)
      setResults(data)
      setLoading(false)
    }, 300)
  ...
  const handleManage = (id: number) => {
    if (type === 'students') router.push(`/students/${id}`)
    else router.push(`/offerings`)
  }

  return (
    <div
      className="rounded-xl shadow flex flex-col"
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => router.push(type === 'students' ? '/students' : '/offerings')}
          ...
          <button
            ...
            style={{ color: showSettings ? '#7c3aed' : 'var(--text-muted)' }}
          ...
          {showSettings && (
            <div
              className="absolute right-0 top-7 rounded-xl shadow-xl z-50 p-4 flex flex-col gap-4"
              style={{
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[{ label: 'Compact', value: 'half' }, { label: 'Full', value: 'full' }].map((s) => (
                    <button
                      key={s.value}
                      onClick={() => update({ size: s.value as any })}
                      className="flex-1 py-1.5 rounded-lg text-xs font-medium transition"
                      style={{
                        background: settings.size === s.value ? '#7c3aed' : 'var(--bg-sub)',
                        color: settings.size === s.value ? '#fff' : 'var(--text-muted)',
                        border: `1px solid ${settings.size === s.value ? '#7c3aed' : 'var(--border)'}`,
                      }}
                    >
                      {s.label}
                    </button>
                ...
                <div className="flex flex-col gap-1">
                  {filters.map((f) => (
                    <button
                      ...
                      style={{
                        background: settings.filter === f.value ? 'rgba(124,58,237,0.08)' : 'transparent',
                        color: settings.filter === f.value ? '#7c3aed' : 'var(--text-muted)',
                      }}
                    ...
                    >
                      <span style={{ fontSize: 8 }}>{settings.filter === f.value ? '●' : '○'}</span>
                      {f.label}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                {!confirmRemove ? (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => { toggleFav(widgetId); setShowSettings(false) }}
      ...
      <div
        ...
        style={{
          gridTemplateColumns: `repeat(${cols.length}, 1fr) 24px`,
          borderBottom: '1px solid var(--border)',
          background: 'var(--bg-sub)',
        }}
      >
        {cols.map((col) => (
          <span key={col} className="text-xs font-semibold" style={{ color: 'var(--text-muted)' }}>{col}</span>
        ))}
        <span />
      </div>
          ...
          results.map((row) => {
            ...
            return (
              <div key={row.id} style={{ borderBottom: '1px solid var(--border)' }}>
                <div
                  className="grid px-4 py-2 cursor-pointer transition"
                  style={{
                    gridTemplateColumns: `repeat(${cols.length}, 1fr) 24px`,
                ...
                >
                  {row.cells.map((cell, j) => (
                    <span key={j} className="text-xs" style={{ color: j === 0 ? 'var(--text-muted)' : 'var(--text-primary)' }}>
                      {cell}
                    </span>
                  ))}
                  <ChevronDown
