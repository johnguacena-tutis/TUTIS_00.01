...
export default function OfferingEnrolmentModal({
  offeringTitle,
  offeringCode,
  onClose,
}: {
  ...
  const toggleStudent = (s: Student) => {
    setForm((f) => {
      ...
      return {
        ...f,
        students: exists
          ? f.students.filter((x) => x.person_id !== s.person_id)
          : [...f.students, s],
      }
    })
  }
  ...
  const filtered = students
    .filter((s) => {
      if (showSelectedOnly) return isSelected(s.person_id)
      return `${s.first_name ?? ''} ${s.surname ?? ''} ${s.code ?? ''}`.toLowerCase().includes(search.toLowerCase())
    })
    .sort((a, b) => (isSelected(a.person_id) ? 0 : 1) - (isSelected(b.person_id) ? 0 : 1))

  const canNext =
    (step === 0 && form.students.length > 0) ||
      ...
      >
            ...
            {steps.map((s, i) => (
              ...
              >
                ...
                <span
                  ...
                  style={{ color: i === step ? '#7c3aed' : i <= maxStep ? 'var(--text-primary)' : 'var(--text-muted)' }}
                >
                  {s}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between mb-3 gap-3">
                ...
                {form.students.length > 0 && (
                  <button
                    ...
                    style={{
                      background: showSelectedOnly ? '#7c3aed' : 'rgba(124,58,237,0.1)',
                      color: showSelectedOnly ? '#fff' : '#7c3aed',
                    }}
                  >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setForm((f) => ({
                                  ...f,
                                  students: [...f.students, ...filtered.filter((s) => !isSelected(s.person_id))],
                                }))
                              } else {
                                setForm((f) => ({
                                  ...f,
                                  students: f.students.filter((s) => !filtered.find((x) => x.person_id === s.person_id)),
                                }))
                              }
                            }}
                          />
                    ...
                    <tbody>
                      {filtered.length === 0 ? (
                        ...
                        filtered.map((s, idx) => {
                          ...
                          const prevSelected = idx > 0 && isSelected(filtered[idx - 1].person_id)
                          const showDivider = !selected && prevSelected
                          return (
                            <>
                            {showDivider && (
                              <tr key={`divider-${s.person_id}`}>
                                <td colSpan={5} className="px-4 py-1 text-xs font-semibold" style={{ background: 'var(--bg-sub)', color: 'var(--text-muted)', borderBottom: '1px solid var(--border)' }}>
                                  Other Students
                                </td>
                              </tr>
                            )}
                  ...
                  {form.students.map((s) => (
                    ...
                    >
                      <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
                        {[s.first_name, s.surname].filter(Boolean).join(' ')}
                      </span>
        ...
        >
          ...
          {step < steps.length - 1 ? (
            ...
            >
              <Check size={14} />
              Create Enrolment{form.students.length > 1 ? 's' : ''}
            </button>
          )}
        </div>
      </div>
