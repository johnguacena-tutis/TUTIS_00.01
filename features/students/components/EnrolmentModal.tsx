...
export default function EnrolmentModal({ studentName, onClose }: { studentName: string; onClose: () => void }) {
  ...
  useEffect(() => {
    async function fetchOfferings() {
      ...
      const active = (versions ?? []).filter((v: any) =>
        activeIds.has(v.offering_id) && (v.end_timestamp === null || new Date(v.end_timestamp) >= now)
      )
      ...
      setOfferings(active.map((v: any) => ({
        offering_version_id: v.offering_version_id, offering_id: v.offering_id,
        code: v.code, title: v.title, start_timestamp: v.start_timestamp, end_timestamp: v.end_timestamp,
  ...
  const canNext = (step === 0 && form.offering !== null) || (step === 1 && form.start_date !== '') || step === 2
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
              {form.offering ? <>Enrolling <span style={{ color: '#7c3aed', fontWeight: 600 }}>{studentName}</span></> : studentName}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
              ...
              {loading ? (
                ...
                <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.length === 0 ? (
                        <tr><td colSpan={5} className="px-4 py-6 text-center" style={{ color: 'var(--text-muted)' }}>No offerings found</td></tr>
                      ) : filtered.map((o) => {
                        const selected = form.offering?.offering_version_id === o.offering_version_id
                        return (
                          <tr key={o.offering_version_id} onClick={() => setForm((f) => ({ ...f, offering: o }))}
                            style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer', background: selected ? 'rgba(124,58,237,0.08)' : 'transparent' }}
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Please review the enrolment details before confirming.</p>
              <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                {[
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          {step < steps.length - 1 ? (
            ...
            <button onClick={onClose} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white transition"
              ...
              <Check size={14} />Create Enrolment
            </button>
          )}
        </div>
      </div>
