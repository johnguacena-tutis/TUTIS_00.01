...
import type { StudentDetail as StudentDetailType, Enrolment } from '../types'
...
const inputStyle = {
  width: '100%', padding: '6px 10px', fontSize: '14px', borderRadius: '6px',
  border: '1px solid #7c3aed', background: 'var(--bg-page)', color: 'var(--text-primary)', outline: 'none',
}

function Field({ label, value, editing, onChange, readOnly = false }: {
  label: string; value: string | null; editing: boolean; onChange?: (v: string) => void; readOnly?: boolean
}) {
  return (
    <div className="flex items-center py-3" style={{ borderBottom: '1px solid var(--border)' }}>
      <span className="text-sm font-medium w-48 shrink-0" style={{ color: 'var(--text-muted)' }}>{label}</span>
      {editing && !readOnly ? (
...
export default function StudentDetail({
  student, enrolments, initialEditing = false,
}: {
  ...
  const handleCancel = () => { setDraft({ ...student }); setIsEditing(false) }
    ...
    <div className="pb-24">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>{draft.code ?? '—'}</p>
        </div>
        <div className="flex items-center gap-2">
          {isEditing ? (
            <button onClick={handleCancel} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition"
            ...
            <>
              {canAddEnrolment && (
                <button onClick={() => setShowEnrolModal(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition"
                  ...
                  <UserPlus size={14} />Add Enrolment
                </button>
              )}
              {canEdit && (
                <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white transition"
      ...
      <div className="rounded-xl shadow" style={{ background: 'var(--bg-sidebar)', border: `1px solid ${isEditing ? '#7c3aed' : 'var(--border)'}` }}>
        <div className="flex" style={{ borderBottom: '1px solid var(--border)' }}>
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className="px-5 py-3 text-sm font-medium transition"
              style={{ color: activeTab === tab ? '#7c3aed' : 'var(--text-muted)', borderBottom: activeTab === tab ? '2px solid #7c3aed' : '2px solid transparent', marginBottom: '-1px', background: 'transparent' }}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'Details' && (
          <div className="p-6">
            ...
            <Field label="Email"          value={draft.email ?? null}   editing={isEditing} onChange={set('email')} />
          </div>
        )}
                ...
                <thead>
                  <tr style={{ background: 'var(--bg-sub)', borderBottom: '1px solid var(--border)' }}>
                    {['Code', 'Qualification / Course', 'Offering', 'Trainer', 'Started', 'End Date', 'Status'].map((col) => (
                      <th key={col} className="text-left px-4 py-3 font-semibold" style={{ color: 'var(--text-muted)' }}>{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {enrolments.length === 0 ? (
                    <tr><td colSpan={7} className="px-4 py-8 text-center" style={{ color: 'var(--text-muted)' }}>No enrolments found</td></tr>
                  ) : (
                    enrolments.map((e) => (
                      <tr key={e.enrolment_id} style={{ borderBottom: '1px solid var(--border)' }}
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 rounded-full text-xs font-semibold capitalize" style={statusColor(e.status)}>
                            {e.status || '—'}
                          </span>
                        </td>
                      </tr>
      ...
      {showEnrolModal && <EnrolmentModal studentName={fullName} onClose={() => setShowEnrolModal(false)} />}

      {isEditing && (
        <div className="fixed bottom-0 left-0 right-0 flex items-center justify-end gap-3 px-8 py-4 shadow-lg"
          style={{ background: 'var(--bg-sidebar)', borderTop: '1px solid var(--border)', zIndex: 50 }}>
          <p className="text-sm mr-auto" style={{ color: 'var(--text-muted)' }}>You have unsaved changes</p>
          <button onClick={handleCancel} className="px-4 py-2 rounded-lg text-sm font-semibold transition"
            style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-muted)' }}
            onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--bg-sidebar-hover)')}
            onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>Cancel</button>
          <button onClick={handleSave} className="px-5 py-2 rounded-lg text-sm font-semibold text-white transition"
            style={{ background: '#7c3aed' }}
            onMouseEnter={(e) => (e.currentTarget.style.background = '#6d28d9')}
